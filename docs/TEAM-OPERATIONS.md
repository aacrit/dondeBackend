...
Step 2: COO spawns reservation-integration-specialist:
        Agent(name: "reservations", subagent_type: "reservation-integration-specialist",
...
Step 4: COO spawns db-reviewer for schema:
        Agent(name: "data", subagent_type: "db-reviewer",
              prompt: "Create restaurant_reservations table migration.
                       Schema: restaurant_id, platform, slug, url, priority, verified.",
              team_name: "donde-integrations-foxtrot")

Step 5: COO spawns frontend-builder for UI:
        Agent(name: "builder", subagent_type: "frontend-builder",
              prompt: "Build 'Reserve' button component for recommendation card.
                       Show platform logo. Handle mobile deep links.
                       Fallback to phone number for walk-in restaurants.",
              team_name: "donde-integrations-foxtrot")

Step 6: COO spawns continuous-tester to verify no scoring regression:
        Agent(name: "tester", subagent_type: "continuous-tester",
              prompt: "Run golden dataset. Verify reservation_links addition
                       does not affect DondeMatch scores.",
              team_name: "donde-integrations-foxtrot")

Step 7: COO aggregates -> CEO Briefing -> Shutdown -> TeamDelete
```
...
- $0 total cost
- Reservation click rate > 20% of recommendations (post-launch)
