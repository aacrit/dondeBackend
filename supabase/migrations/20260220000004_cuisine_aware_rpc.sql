-- Cuisine-aware RPC: adds p_target_cuisine parameter for intent-based ranking
-- When a target cuisine is provided, matching restaurants are boosted to the top of results
-- Uses ORDER BY boosting (not WHERE filtering) so non-matching restaurants still appear as fallback
...
BEGIN
    ...
    IF v_use_total THEN
        ...
        ORDER BY
            -- Cuisine boost: matching restaurants float to top when target cuisine is specified
