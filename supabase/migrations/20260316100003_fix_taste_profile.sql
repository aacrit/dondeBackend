-- Fix compute_taste_profile JOIN bug: noise_level and dress_code are on
-- the restaurants table, NOT restaurant_deep_profiles. The liked_vibes CTE
-- was joining only restaurant_deep_profiles (dp) but referencing dp.noise_level
-- and dp.dress_code which don't exist on that table. This fix adds a JOIN to
-- the restaurants table and changes the column references accordingly.
-- energy_level remains on dp (restaurant_deep_profiles) where it belongs.
...
BEGIN
  ...
  -- FIX: noise_level and dress_code are on restaurants (r), not restaurant_deep_profiles (dp)
  -- energy_level remains on dp where it belongs
  WITH liked_vibes AS (
    SELECT
      r.noise_level,
      dp.energy_level,
