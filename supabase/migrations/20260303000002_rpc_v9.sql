-- V9: Two-Stage Retrieval RPC
-- Stage 1: Broad retrieval using review intelligence text search (no hard cuisine filter)
-- Returns top 50 candidates ranked by text relevance + occasion score
-- Stage 2: Full V9 scoring happens in TypeScript

CREATE OR REPLACE FUNCTION get_candidates_v9(
...
RETURNS TABLE (
  ...
  -- Review intelligence fields (NEW in V9)
...
BEGIN
  ...
  SELECT
    ...
    -- Occasion score: use occasion-specific column or average
    CASE
    ...
    -- Full-text search rank (0-1): how well the query matches review intelligence
  ...
  WHERE r.noise_level IS NOT NULL
    ...
    -- V9: NO hard cuisine filter — review intelligence + TypeScript scoring handles relevance
    -- Soft neighborhood preference: restaurants in the requested neighborhood are preferred
    -- but NOT excluded if outside it (V9 post-scoring gate handles this)
  ORDER BY
    -- Primary: Review intelligence text match (dish/cuisine from reviews)
    CASE WHEN p_query IS NOT NULL AND ri.review_search_vector IS NOT NULL
         THEN ts_rank(ri.review_search_vector, plainto_tsquery('english', p_query))
         ELSE 0.0
    END DESC,
    -- Secondary: Neighborhood preference (in-neighborhood ranked higher)
    CASE WHEN v_neighborhood_id IS NOT NULL AND r.neighborhood_id = v_neighborhood_id
         THEN 1 ELSE 0
    END DESC,
    -- Tertiary: Total occasion score as tiebreaker
    (COALESCE(os.date_friendly_score, 0) + COALESCE(os.group_friendly_score, 0) +
     COALESCE(os.family_friendly_score, 0) + COALESCE(os.romantic_rating, 0) +
     COALESCE(os.business_lunch_score, 0) + COALESCE(os.solo_dining_score, 0) +
