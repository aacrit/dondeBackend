...
// V19: bug-fixer — Fixed 10 canonical names to match DB neighborhoods table:
// "The Loop" → "Loop", "University Village" → "Little Italy", "Lincoln Park" → "Old Town" (for old town alias),
// "Albany Park" → "Irving Park", "Little Village" → "Pilsen", "Portage Park" → "Irving Park",
// "O'Hare" → "Irving Park", "Midway" → "Bridgeport", "Garfield Park" → "Humboldt Park",
// "Oak Park" → "Humboldt Park", "Evanston" → "Rogers Park"
export const NEIGHBORHOOD_ALIASES: Record<string, string> = {
  ...
  "o'hare": "Irving Park",
  "o'hare airport": "Irving Park",
  "near o'hare": "Irving Park",
  "near o'hare airport": "Irving Park",
  "midway": "Bridgeport",
  "midway airport": "Bridgeport",
  "near midway": "Bridgeport",
  "near midway airport": "Bridgeport",
  "union station": "Loop",
  "near union station": "Loop",
  "union loop cta": "Loop",
  "near union loop cta": "Loop",
  // V13: Museums & Cultural Venues
  "art institute": "Loop",
  "art institute chicago": "Loop",
  "near art institute": "Loop",
  "near art institute chicago": "Loop",
  ...
  "near chicago cultural center": "Loop",
  "auditorium theatre": "Loop",
  "near auditorium theatre": "Loop",
  "chicago theatre": "Loop",
  "near chicago theatre": "Loop",
  "lyric opera": "Loop",
  "near lyric opera": "Loop",
  ...
  "oak park": "Humboldt Park",
  "near oak park": "Humboldt Park",
  "evanston": "Rogers Park",
  "accessible from evanston": "Rogers Park",
  "printers row": "South Loop",
  "printers row restaurant": "South Loop",
  "near randolph restaurant row": "West Loop",
  "divvy": "Loop",
  "divvy station": "Loop",
  "near divvy stations": "Loop",
  "divvy bike nearby": "Loop",
  "near expressway exit": "West Loop",
  "close to the loop financial": "Loop",
  "close to the loop": "Loop",
  // V13: Universities
...
export const CONCEPT_MAP: Record<string, ConceptSignal> = {
  ...
  "pre-game": { neighborhoods: ["West Loop", "Lakeview", "South Loop"], tags: ["lively atmosphere"], constraints: ["walk_in"], vibes: ["lively", "buzzing"] },
  "pre-game dinner": { neighborhoods: ["West Loop", "Lakeview", "South Loop"], tags: ["lively atmosphere"], constraints: ["walk_in"], vibes: ["lively"] },
  "pre-concert": { neighborhoods: ["West Loop", "Loop"], tags: ["lively atmosphere"], constraints: ["walk_in"] },
  "post-theater": { neighborhoods: ["Loop", "River North"], tags: ["late night", "craft cocktails"], vibes: ["elegant", "refined"] },
  ...
  "little italy food": { neighborhoods: ["Little Italy"], cuisines: ["Italian"] },
  ...
  if (isReputationQuery(intent, specialRequest)) {
    ...
    if (intent) {
      ...
      // LOOSE: vegetarian friendly — many restaurants can be vegetarian-friendly without being dedicated.
      {
        ...
        // DEDICATED vegan/GF spot, not just any restaurant that accommodates the diet.
        // V25: Only cuisine_type or restaurant name determines "dedicated" status.
        // Tags like "gluten-free" and dietary_options like "Gluten-Free" are too broad —
        // many steakhouses and fine dining restaurants list these as accommodations.
        // A dedicated restaurant has the dietary type IN its cuisine_type (e.g., "Vegan")
        // or IN its name (e.g., "Wheat's End Cafe", "PLANTA Queen", "Chicago Diner").
        ...
        for (const [queryKw, dietaryTerms] of DEDICATED_DIETARY) {
          if (specialLower.includes(queryKw)) {
            const nameL = (candidate.name || "").toLowerCase();
            const isDedicated =
              // Cuisine type IS the dietary type (strongest signal)
              dietaryTerms.some(dt => cuisineType.includes(dt)) ||
              // Restaurant name signals dedication
              nameL.includes(dietaryTerms[0].split("-")[0]) || // "gluten" in name, "vegan" in name
              nameL.includes("plant") || // plant-based restaurants
              // Oneliner makes dietary identity a PRIMARY feature (not just an option)
              dietaryTerms.some(dt => {
                // Oneliner must contain the term as a PRIMARY descriptor, not just "offers X options"
                const inOneliner = oneliner.includes(dt);
                const isJustOption = oneliner.includes(dt + " option") || oneliner.includes(dt + " menu");
                return inOneliner && !isJustOption;
              });
            if (!isDedicated) {
